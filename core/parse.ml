...
let reader_of_channel channel =
  ...
  fun buffer nchars ->
    ...
    lexbuf.lex_curr_p <- { lexbuf.lex_curr_p with pos_cnum = !current_pos - lexbuf.lex_curr_p.pos_bol };
    nchars

(* Given a string, return a function suitable for input to
   Lexing.from_function that reads characters from the string.
*)
(* Original reader_of_string *)
let reader_of_string ?pp string =
  Printf.printf("reader_of_string called");
  flush stdout;
...
(* let reader_of_string ?pp string =
  ...
  let current_pos = ref 0 in
  let line_start = ref 0 in
  let string_bytes = Bytes.of_string string in
  fun buffer nchars ->
    let nchars = min nchars (Bytes.length string_bytes - !current_pos) in
    Bytes.blit string_bytes !current_pos buffer 0 nchars;
    for i = 0 to nchars - 1 do
      if Bytes.get buffer i = '\n' then
        line_start := !current_pos + i + 1
    done;
...
let parse_file ?(pp=default_preprocessor ()) ?in_context:context grammar filename =
  ...
  match normalize_pp pp with
    ...
    | Some pp ->
        Printf.printf("[parse] parse_file called (after match)\n");
        flush stdout;
